function [Sigma,S,CostNew] = graphupdate(KH,Sigma,GradNew,CostNew,option)
...
while costmax<costmin
    [costmax, S] = costgraph(KH,stepmax,desc,SigmaNew);
...
while (stepmax-stepmin)>option.goldensearch_deltmax*(abs(deltmax)) && stepmax > eps
    ...
    [costmedr, S1] = costgraph(KH,stepmedr,desc,SigmaNew);
    [costmedl, S2] = costgraph(KH,stepmedl,desc,SigmaNew);     
